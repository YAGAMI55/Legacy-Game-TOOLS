...
def from_bits(bits):
    ...
    return bytes(
        int(bits[b * 8:(b + 1) * 8], 2)
        for b in range(bit_length // 8)
    )
...
def read_string(f):
    ...
    if is_unicode:
        string = ''.join(chr(read_u16_le(f)) for _ in range(-count))
    else:
        string = ''.join(chr(read_u8(f)) for _ in range(count))
...
def write_string(f, string):
    ...
    if ENDIAN == '>':
        write_s32_be(f, -count if any(ord(c) > 255 for c in string) else count)
    else:
        write_s32_le(f, -count if any(ord(c) > 255 for c in string) else count)

    is_unicode = any(ord(c) > 255 for c in string)
...
class IniFile(object):
    ...
    def load(self, f):
        ...
        for line in f:
            ...
            key, value = m.group(1), unescape(
                m.group(2),
                self.enclose_in_quotes
            )
        ...
        for section in self.sections:
            ...
            for (key, value) in section.pairs:
                f.write(
                    '{0}={1}\r\n'.format(
                        key,
                        escape(value, self.enclose_in_quotes)
                    )
                )
...
class CoalescedFile(object):
    ...
    class CodeTable(object):
        ...
        class Node(object):
            ...
            def __repr__(self):
                if self.is_leaf():
                    return 'Leaf(symbol={0}, frequency={1})'.format(
                        self.symbol,
                        self.frequency
                    )
                else:
                    return 'Branch(frequency={0})'.format(
                        self.frequency
                    )
        ...
        class Code(object):
            ...
            def dump(self, f, i, depth):
                print(
                    indentation(depth) +
                    'code: {0}: symbol:{1} left:{2} right:{3}'.format(
                        i,
                        self.symbol,
                        self.left,
                        self.right
        ...
        def save(self, f):
            ...
            def build_codes(node):
                if node.is_leaf():
                    ...
                    code = self.Code()
                    code.symbol = 0xFFFF
                    code.left = self.nodes.index(node.left) \
                        if node.left is not None else -1
                    code.right = self.nodes.index(node.right) \
        ...
        def encode(self, string):
            return ''.join(self.codes[ord(c)] for c in string)
        ...
        def decode(self, bits):
            ...
            for bit in bits:
                ...
                if node.is_leaf():
                    data += chr(node.symbol)
                    node = self.tree

            return data

        def dump(self, f, depth):
            print(indentation(depth) + 'codes:', file=f)

            for symbol in sorted(
                self.codes,
                key=lambda x: len(self.codes[x])
            ):
                print(
                    indentation(depth + 1) +
                    'symbol:{0} code:{1}'.format(
                        symbol,
                        self.codes[symbol]
                    ),
                    file=f
        ...
        class Section(object):
            ...
            class Pair(object):
                ...
                def load(self, f):
                    ...
                    if ENDIAN == '>':
                        self.length = (data[0] << 8) | data[1]
                        data = data[2:]
                    else:
                        self.length = (data[1] << 8) | data[0]
                        data = data[2:]

                    if self.length > 0:
                        self.bits = strip_leading_zeroes(
                            to_bits(data[::-1])
                        )[::-1]
                    else:
                        self.bits = ''
                ...
                def decode(self, code_table):
                    self.value = code_table.decode(self.bits) \
                        if self.length > 0 else ''

                def encode(self, code_table):
                    self.bits = code_table.encode(self.value) \
                        if self.length > 0 else ''
...
if command == 'unpack':
    ...
    for file_name in bin_files:
        ...
        coalesced_file = CoalescedFile(
            file_base.upper().startswith('COALESCED_')
        )
    ...
    for file_name, coalesced_file in localization_files.items():
        ...
        for file in coalesced_file.files:

            file_path = os.path.join(
                output_directory,
                file.path.replace('..\\', '').replace('\\', os.sep)
            )
            ...
            for section in file.sections:
                ...
                for pair in section.pairs:
                    ...
                    value = pair.value[:pair.length - 1] \
                        if pair.length > 0 else ''
    ...
    with codecs.open(listing_file_path, 'r', 'utf-8') as f:
        ...
        for line in lines:
            ...
            file_name = parts[0]
            ini_files = py2_filter_bool(parts[1].split(':'))
            ...
            for ini_file_path in ini_files:
                ...
                file_path = os.path.join(
                    input_directory,
                    ini_file_path.replace('..\\', '').replace('\\', os.sep)
                )
    ...
    for file_name, coalesced_file in localization_files.items():
        symbols += ''.join(
            list(coalesced_file.iterate_over_chars())
        )
    ...
    for file_name in bin_files:
        ...
        localization_files[file_name] = ':'.join(
            [x.path for x in coalesced_file.files]
        )
