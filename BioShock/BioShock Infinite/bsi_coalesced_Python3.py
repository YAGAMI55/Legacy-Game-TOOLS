...
if len(sys.argv) < 2:
	print('Coalesced tool for BioShock Infinite (PS3)')
...
def read_string(f):
	...
	is_unicode = count < 0
	if is_unicode:
		string = ''.join(chr(read_u16_le(f)) for x in range(-count))
	else:
		string = ''.join(chr(read_u8(f)) for x in range(count))
...
class CoalescedFile(object):
	class CodeTable(object):
		...
		class Code(object):
			...
			def dump(self, f, i, depth):
				print(indentation(depth) + 'code: {0}: symbol:{1} left:{2} right:{3}'.format(i, self.symbol, self.left, self.right), file=f)
		...
		def compute(self, symbols):
			...
			for c in symbols:
				frequencies[ord(c)] += 1
			self.tree = [self.Node(symbol, frequency) for symbol, frequency in list(frequencies.items())]
		...
		def dump(self, f, depth):
			...
			for symbol in sorted(self.codes, key=lambda x: len(self.codes[x])):
				print(indentation(depth + 1) + 'symbol:{0} code:{1}'.format(symbol, self.codes[symbol]), file=f)
	...
	class File(object):
		class Section(object):
			class Pair(object):
				...
				def dump(self, f, depth):
					if self.value is None:
						print(indentation(depth) + 'key:{0} bits:{1}'.format(self.key, self.bits), file=f)
					else:
						value = escape(self.value.rstrip('\0'))
						print(indentation(depth) + 'key:{0} value:{1}'.format(self.key, value), file=f)
			...
			def dump(self, f, depth):
				print(indentation(depth) + 'name:{0} num_pairs:{1}'.format(self.name, self.num_pairs), file=f)
				print(indentation(depth) + 'pairs:', file=f)
				for i in range(self.num_pairs):
					pair = self.pairs[i]
					pair.dump(f, depth + 1)
		...
		def dump(self, f, depth):
			print(indentation(depth) + 'path:{0} num_sections:{1}'.format(self.path, self.num_sections), file=f)
			print(indentation(depth) + 'sections:', file=f)
			for i in range(self.num_sections):
				section = self.sections[i]
	...
	def iterate_over_values(self):
		for i in range(self.num_files):
			file = self.files[i]
			for j in range(file.num_sections):
				section = file.sections[j]
				for k in range(section.num_pairs):
	...
	def dump(self, f, depth=0):
		if self.is_global:
			print(indentation(depth) + 'code_table:', file=f)
			self.code_table.dump(f, depth + 1)
		print(indentation(depth) + 'num_files:{0}'.format(self.num_files), file=f)
		print(indentation(depth) + 'files:', file=f)
		for i in range(self.num_files):
			file = self.files[i]
			file.dump(f, depth + 1)
...
if command == 'unpack':
	...
	for file_name, coalesced_file in list(localization_files.items()):
		coalesced_file.decode(code_table)
		if log_file is not None:
			log_file.write('{0}:\n'.format(file_name))
			coalesced_file.dump(log_file)
			log_file.write('\n--------------------\n\n')
		for i in range(coalesced_file.num_files):
	...
	with codecs.open(listing_file_path, 'r', 'utf-8') as f:
		...
		for line in lines:
			...
			if len(parts) < 2:
				continue
			file_name, ini_files = parts[0], list(filter(bool, parts[1].split(':')))
			file_base, file_extension = os.path.splitext(file_name)
